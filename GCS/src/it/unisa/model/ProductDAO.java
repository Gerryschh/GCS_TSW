...
public class ProductDAO{
	...
	public static synchronized void doSave(ProductBean product) throws SQLException {
		...
		String insertSQL = "INSERT INTO " + ProductDAO.TABLE_NAME
				+ " (nome, descrizione, prezzo, quantità, thumbnail, sconto, iva, categoria) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			preparedStatement.setString(8, product.getCategoria());
	...
	public static synchronized boolean doUpdate(ProductBean bean) throws SQLException {
		...
		String deleteSQL = "update " + ProductDAO.TABLE_NAME + " SET nome= ?, prezzo= ?, descrizione= ?, sconto= ?, prezzo_scontato= ?, "
				+ " iva= ?, quantità= ? categoria= ? WHERE id = ?";
		...
		try {
			...
			preparedStatement.setDouble(6, bean.getIva());
			preparedStatement.setInt(7, bean.getQuantity());
			preparedStatement.setInt(8, bean.getCode());
			preparedStatement.setString(9, bean.getCategoria());
			result = preparedStatement.executeUpdate();
		} finally {
			try {
				if (preparedStatement != null)
